-- Debug helpers for dispatch/provider discovery.
-- These are intentionally SECURITY DEFINER so Edge Functions (service_role) can call them,
-- and admins can use them in SQL editor when needed.
...
AS $$
  ...
  calc AS (
    SELECT
      ...
      (
        (b.loc_updated_at IS NOT NULL AND b.loc_updated_at > now() - interval '5 minutes')
        OR (b.last_seen_at IS NOT NULL AND b.last_seen_at > now() - interval '5 minutes')
      ) AS online_ok,
      (
        6371 * acos(
          LEAST(
