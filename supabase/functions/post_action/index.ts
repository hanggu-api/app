...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!commandKey || !screenKey) {
      return json({
        success: false,
        message: "command_key e screen_key são obrigatórios",
      }, 400);
    }
    ...
    if (!isCommandAllowed(commandKey)) {
      ...
      return json({
        success: false,
        message: `Comando '${commandKey}' não reconhecido`,
      }, 403);
    }
    ...
    if (policy && !policy.is_allowed) {
      await writeAuditLog(client, {
        ...
        store_version: body.store_version,
        ...
        result_message: "blocked_by_policy",
      });
      return json({
        success: false,
        message: "Ação não permitida neste contexto",
      }, 403);
    }
...
async function executeCommand(
  ...
  switch (commandKey) {
    case "cancel_service_request": {
      const serviceId = entityIds["service_id"]?.toString() ??
        args["service_id"]?.toString();
      if (!serviceId) return fail("service_id obrigatório para cancelar");
      const { data: service } = await client
        .from("service_requests").select("id,status").eq("id", serviceId)
        .maybeSingle();
      if (!service) return fail("Serviço não encontrado");
      const terminal = ["completed", "cancelled", "canceled", "refunded"];
      if (terminal.includes(service.status)) {
        return fail("Serviço já está em status terminal");
      }
      ...
      return ok("Serviço cancelado com sucesso", {
        refresh_screen: true,
        updated_state: { service_id: serviceId, status: "cancelled" },
      });
    }
    ...
    case "refresh_search_status": {
      ...
      const { data: service } = await client
        .from("service_requests").select("id,status,provider_id").eq(
          "id",
          serviceId,
        ).maybeSingle();
      if (!service) return fail("Serviço não encontrado");
    ...
    case "confirm_service_completion": {
      ...
      await client.from("service_requests")
        ...
        .eq("id", serviceId);
      return ok("Conclusão solicitada", {
        refresh_screen: true,
        updated_state: {
          service_id: serviceId,
          status: "completion_requested",
    ...
    case "toggle_dispatch_availability": {
      ...
      return ok(
        ...
        { updated_state: { is_available: isAvailable } },
      );
    }
    ...
    case "accept_ride": {
      ...
      const { data: service } = await client
        ...
        .maybeSingle();
      if (!service) return fail("Serviço não encontrado");
      if (service.status !== "searching") {
        return fail("Serviço não está mais disponível para aceite");
      }
      return ok("Aceite autorizado", {
        effects: [{ type: "accept_service", service_id: serviceId }],
        updated_state: { service_id: serviceId },
      });
    }
    ...
    case "reject_ride": {
      ...
      return ok("Recusa registrada", {
        effects: [{ type: "reject_service", service_id: serviceId }],
      });
    }
    ...
    case "retry_pix_generation": {
      ...
      return ok("PIX autorizado", {
        effects: [{ type: "generate_pix", service_id: serviceId }],
        updated_state: { service_id: serviceId },
      });
    }

    case "confirm_direct_payment_intent": {
      const intentId = entityIds["intent_id"]?.toString() ??
        args["intent_id"]?.toString();
      if (!intentId) return fail("intent_id obrigatório");
      return ok("Intenção confirmada", {
        updated_state: { intent_id: intentId, status: "confirmed" },
      });
    }
    ...
    case "open_pix_screen":
      return ok("Navegação autorizada", {
        effects: [{ type: "navigate", command_key: commandKey, args }],
      });
