...
class _ProviderScheduleSettingsScreenState extends State<ProviderScheduleSettingsScreen> {
  ...
  bool _isValidSchedule(int day) {
    ...
    final isMainValid = (end.hour > start.hour) || (end.hour == start.hour && end.minute > start.minute);
  ...
  Future<void> _save() async {
    ...
    for (int i = 0; i < 7; i++) {
      if (_activeDays[i] == true && !_isValidSchedule(i)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Horário inválido na ${days[i]}. O fim deve ser após o início.'), backgroundColor: Colors.red),
        );
  ...
  Widget _buildTimePicker(TimeOfDay? time, Function(TimeOfDay) onPick, String label, {bool canClear = false}) {
    return GestureDetector(
      onTap: () async {
        final t = await showTimePicker(context: context, initialTime: time ?? const TimeOfDay(hour: 9, minute: 0));
        if (t != null) onPick(t);
      },
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
