...
class _ServiceDetailsScreenState extends State<ServiceDetailsScreen>
  ...
  Future<void> _loadRoute() async {
    ...
    try {
      ...
      if (_service != null &&
          _service!['latitude'] != null &&
          _service!['longitude'] != null) {
        final destLat = double.tryParse(_service!['latitude'].toString()) ?? 0;
        final destLng = double.tryParse(_service!['longitude'].toString()) ?? 0;
        ...
        actions: [
          if (_service != null &&
            ...
            IconButton(
              ...
              onPressed: () => context.push('/chat', extra: widget.serviceId),
            ),
        ],
      ),
      body: _buildBody(),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow(
            ...
            _api.role == 'provider' ? 'Valor a Receber' : 'Valor Total',
            'R\$ ${(double.tryParse((_api.role == 'provider' ? s['provider_amount'] : s['price_estimated'])?.toString() ?? '0') ?? 0).toStringAsFixed(2)}',
          ),
          const SizedBox(height: 16),
          ...
          if (_api.role == 'provider') ...[
            ...
            ListTile(
              ...
              leading: CircleAvatar(
                ...
                child: Text((s['user_name'] ?? 'C')[0].toUpperCase()),
              ),
              title: Text(s['user_name'] ?? 'Cliente'),
  ...
  Widget _buildProofSection() {
    ...
    final proofCode = _service!['proof_code'];
    final proofPhoto = _service!['proof_photo'];

    if (proofCode == null && proofPhoto == null) return const SizedBox.shrink();
      ...
      children: [
        ...
        if (proofPhoto != null)
          FutureBuilder<String>(
            future: _api.getMediaViewUrl(proofPhoto),
              ...
              if (snapshot.hasData) {
                return ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    ...
                    placeholder: (context, url) => BaseSkeleton(height: 200, width: 800),
                    errorWidget: (context, url, error) => const Icon(Icons.error),
                  ),
                );
              }
              return BaseSkeleton(height: 200, width: 800);
            },
          ),
      ],
    );
  ...
  Widget? _buildBottomAction() {
    ...
    if (status == 'waiting_remaining_payment') {
      return Padding(
        padding: const EdgeInsets.all(16),
        child: SizedBox(
          width: double.infinity,
    ...
    if (status == 'in_progress') {
      ...
      final startTime = _serviceStartTime ?? DateTime.now();
      ...
      return Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _ServiceDurationTimer(startTime: startTime),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
...
class _ServiceDurationTimer extends StatefulWidget {
  final DateTime startTime;
  const _ServiceDurationTimer({required this.startTime});

  @override
  State<_ServiceDurationTimer> createState() => _ServiceDurationTimerState();
}

class _ServiceDurationTimerState extends State<_ServiceDurationTimer> {
  late Timer _timer;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        'Tempo de serviço: $hours:$minutes:$seconds',
        style: TextStyle(
          fontSize: 12,
          fontWeight: FontWeight.bold,
