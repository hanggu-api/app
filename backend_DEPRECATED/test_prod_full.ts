...
const BACKEND_URL = "https://backend-sandy-alpha-69.vercel.app/api";
...
async function testProdFull() {
  ...
  try {
    ...
    if (loginRes.data.success) {
      // Backend expects us to reuse the firebase token
      token = "TEST_TOKEN";
      console.log("   ✅ Login successful (using TEST_TOKEN)");
    } else {
    ...
    const aiRes = await axios.post(
      `${BACKEND_URL}/services/ai/classify`,
      {
        text,
      },
      {
