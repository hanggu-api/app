-- Migration: support new scheduling statuses
...
CREATE TABLE "service_requests_new" (
    ...
    "provider_id" BIGINT,
    "description" TEXT,
    "status" TEXT NOT NULL CHECK(status IN ('waiting_payment','pending','accepted','waiting_payment_remaining','in_progress','waiting_client_confirmation','completed','cancelled','contested','expired','open_for_schedule','schedule_proposed','scheduled')) DEFAULT 'waiting_payment',
    "latitude" DECIMAL,
    "longitude" DECIMAL,
    "address" TEXT,
...
CREATE INDEX IF NOT EXISTS "idx_service_requests_scheduled_at" ON "service_requests" ("scheduled_at");
