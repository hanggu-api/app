...
// .wrangler/tmp/bundle-ee5b2M/checked-fetch.js
...
async function sendFCMNotificationV1(serviceAccountJson, token, payload) {
  try {
    ...
    const isUrgent = payload.data?.type === "new_service" || payload.data?.type === "offer" || payload.data?.type === "service_offered" || payload.data?.type === "service_dispatch";
    if (isUrgent) {
      console.log("[FCM v1] Urgent offer detected. Converting to DATA-ONLY message for background wake-up.");
      delete messageBody.notification;
      if (messageBody.android) {
        messageBody.android.priority = "HIGH";
        delete messageBody.android.notification;
      }
      if (messageBody.apns) {
    ...
    if (response.ok) {
      ...
      const messageId = responseJson.name;
...
var ServiceStatus = /* @__PURE__ */ ((ServiceStatus2) => {
  ServiceStatus2["WAITING_PAYMENT"] = "waiting_payment";
  ServiceStatus2["PENDING"] = "pending";
  ServiceStatus2["ACCEPTED"] = "accepted";
  ServiceStatus2["WAITING_PAYMENT_REMAINING"] = "waiting_payment_remaining";
  ServiceStatus2["IN_PROGRESS"] = "in_progress";
  ServiceStatus2["WAITING_CLIENT_CONFIRMATION"] = "waiting_client_confirmation";
  ServiceStatus2["COMPLETED"] = "completed";
  ServiceStatus2["CANCELLED"] = "cancelled";
  ServiceStatus2["CONTESTED"] = "contested";
  ServiceStatus2["EXPIRED"] = "expired";
  ServiceStatus2["OPEN_FOR_SCHEDULE"] = "open_for_schedule";
  ServiceStatus2["SCHEDULE_PROPOSED"] = "schedule_proposed";
  ServiceStatus2["SCHEDULED"] = "scheduled";
  return ServiceStatus2;
})(ServiceStatus || {});
var app = new Hono2();
var LOG = {
  sistema: /* @__PURE__ */ __name((msg) => console.log(`[\u2699\uFE0F SISTEMA] ${msg}`), "sistema"),
  auth: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F464} AUTH] ${msg}`), "auth"),
  pagamento: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F4B3} PAGAMENTO] ${msg}`), "pagamento"),
  despacho: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F69A} DESPACHO] ${msg}`), "despacho"),
  notificacao: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F514} NOTIFICA\xC7\xC3O] ${msg}`), "notificacao"),
  servico: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F527} SERVI\xC7O] ${msg}`), "servico"),
  erro: /* @__PURE__ */ __name((msg, detalhe) => {
    ...
    if (detalhe) {
      ...
      if (typeof detalhe === "object" && detalhe !== null) {
        try {
          ...
          console.error(`[\u274C CONTEXTO] (n\xE3o serializ\xE1vel)`);
        }
      }
    }
  }, "erro"),
  sucesso: /* @__PURE__ */ __name((msg) => console.log(`[\u2705 SUCESSO] ${msg}`), "sucesso"),
  debug: /* @__PURE__ */ __name((msg) => console.log(`[\u{1F50D} DEBUG] ${msg}`), "debug"),
  warn: /* @__PURE__ */ __name((msg) => console.warn(`[\u26A0\uFE0F ATEN\xC7\xC3O] ${msg}`), "warn")
};
app.use("/*", cors({
  origin: "*",
...
app.post("/api/debug/force-dispatch", async (c) => {
  try {
    ...
    LOG.despacho(`For\xE7ado ciclo de despacho via endpoint de debug`);
    return c.json({ success: true, message: "Dispatch forced via DO fetch", do_status: res.status });
  } catch (e) {
    LOG.erro(`Falha ao for\xE7ar despacho:`, e);
    return c.json({ success: false, error: e.message }, 500);
  }
});
app.post("/api/test/force-payment-approval", async (c) => {
  const testSecret = c.req.header("X-Test-Secret");
  if (testSecret !== c.env.TEST_SECRET && testSecret !== "maestro-v2-test-secret") {
    return c.json({ success: false, message: "Unauthorized Test Access" }, 401);
  }
  const { service_id, type } = await c.req.json();
  const db = c.env.DB;
  try {
    const service = await db.prepare("SELECT status, provider_id, client_id, arrived_at FROM service_requests_new WHERE id = ?").bind(service_id).first();
    ...
    if (type === "initial" && service.status === "waiting_payment") {
      ...
      updateSql = "UPDATE service_requests_new SET status = ? WHERE id = ?";
      params = [newStatus, service_id];
      if (newStatus === "pending") {
        c.executionCtx.waitUntil((async () => {
          const updatedService = await db.prepare("SELECT * FROM service_requests_new WHERE id = ?").bind(service_id).first();
          if (updatedService) {
      ...
      updateSql = "UPDATE service_requests_new SET status = ?, payment_remaining_status = ? WHERE id = ?";
      params = ["in_progress" /* IN_PROGRESS */, "paid", service_id];
    }
    if (updateSql) {
...
app.get("/api/services/professions", async (c) => {
  try {
    ...
    LOG.erro("Erro ao carregar mapa de profiss\xF5es:", error);
...
async function triggerServiceNotifications(serviceId, db, env, executionCtx) {
  try {
    LOG.despacho(`\u2699\uFE0F ====== Iniciando Acordada do Despachante para Servi\xE7o: ${serviceId} ======`);
    ...
    LOG.despacho(`\u{1F514} Sinal de WAKE-UP enviado ao Despachante Global.`);
  } catch (error) {
    LOG.erro(`Erro ao acordar despachante para servi\xE7o ${serviceId}:`, error);
  }
}
__name(triggerServiceNotifications, "triggerServiceNotifications");
async function syncStatusToFirebase(env, serviceId, status, userIds = [], additionalData = {}) {
  const serviceAccountJson = env.FIREBASE_SERVICE_ACCOUNT;
  if (!serviceAccountJson) {
    LOG.warn(`[Sync] Sincroniza\xE7\xE3o omitida: FIREBASE_SERVICE_ACCOUNT n\xE3o configurado`);
    return;
  }
  try {
    const serviceAccount = JSON.parse(serviceAccountJson);
    ...
    const accessToken = await getAccessTokenFromServiceAccount(serviceAccountJson);
    if (!accessToken) {
      LOG.erro(`[Sync] Falha ao obter token OAuth para sincroniza\xE7\xE3o`);
      return;
    }
    ...
    LOG.erro(`[Sync] Erro cr\xEDtico: ${e.message}`);
  }
}
__name(syncStatusToFirebase, "syncStatusToFirebase");
async function updateServiceStatus(env, serviceId, newStatus, userIdsToNotify = [], additionalData = {}, extraUpdates = {}, whereClause = "id = ?", whereBinds = []) {
  const db = env.DB;
  try {
    let query = 'UPDATE service_requests_new SET status = ?, updated_at = datetime("now")';
    ...
    if (result.meta.changes > 0) {
      LOG.sucesso(`[Maestro] Status do servi\xE7o ${serviceId} atualizado para ${newStatus} no D1`);
      await syncStatusToFirebase(env, serviceId, newStatus, userIdsToNotify, additionalData);
    }
    return result;
  } catch (e) {
    LOG.erro(`[Maestro] Falha ao atualizar status do servi\xE7o ${serviceId}: ${e.message}`);
    throw e;
  }
}
__name(updateServiceStatus, "updateServiceStatus");
async function findProvidersByDistance(db, profession, latitude, longitude, maxRadiusKm = 50) {
  try {
    console.log(`[Log\xEDstica] \u{1F50D} ====== Buscando Prestadores via Registry (Otimizado v9) ======`);
    console.log(`[Log\xEDstica] Profiss\xE3o: ${profession}`);
    console.log(`[Log\xEDstica] Localiza\xE7\xE3o: ${latitude}, ${longitude}`);
    ...
    if (!result.results || result.results.length === 0) {
      console.log(`[Log\xEDstica] \u26A0\uFE0F Nenhum registro encontrado para a profiss\xE3o: ${profession}`);
      return [];
    }
    ...
    console.log(`[Log\xEDstica] \u2705 Encontrados ${providers.length} prestadores via SQL`);
    return providers;
  } catch (error) {
    console.error("[Log\xEDstica] \u274C Erro SQL:", error.message);
    return [];
  }
}
__name(findProvidersByDistance, "findProvidersByDistance");
function calculateProviderAmount(totalAmount, commissionRate) {
  return totalAmount * (1 - commissionRate);
}
__name(calculateProviderAmount, "calculateProviderAmount");
async function getPlatformCommission(db) {
  try {
    const comm = await db.prepare("SELECT value FROM app_config WHERE key = 'platform_commission_rate'").first();
    return parseFloat(comm?.value || "15") / 100;
  } catch (e) {
    return 0.15;
  }
}
__name(getPlatformCommission, "getPlatformCommission");
async function populateServiceQueue(db, serviceId, serviceData) {
  LOG.despacho(`=== INICIANDO POPULA\xC7\xC3O DA FILA PARA SERVI\xC7O ${serviceId} ===`);
  const commissionRate = await getPlatformCommission(db);
  LOG.despacho(`Taxa de comiss\xE3o da plataforma: ${(commissionRate * 100).toFixed(1)}%`);
  ...
  const { results: candidates } = await db.prepare(`
    SELECT
      ...
      nr.radius_km,
      (6371 * acos(
        cos(radians(?1)) * cos(radians(nr.latitude)) *
        cos(radians(nr.longitude) - radians(?2)) +
        sin(radians(?1)) * sin(radians(nr.latitude))
      )) AS distance
    ...
    WHERE nr.fcm_token IS NOT NULL
      ...
      AND (
        (nr.professions_ids IS NOT NULL 
         ...
         AND EXISTS (SELECT 1 FROM json_each(nr.professions_ids) WHERE value = ?3))
        OR (nr.professions_ids LIKE ?4)
      )
      AND nr.latitude BETWEEN ?5 AND ?6
      AND nr.longitude BETWEEN ?7 AND ?8
    ORDER BY distance ASC
  `).bind(
  ...
  if (!candidates || candidates.length === 0) {
    LOG.warn(`Nenhum candidato encontrado para o servi\xE7o ${serviceId} (Profiss\xE3o ID: ${targetId})`);
    return 0;
  }
  LOG.despacho(`Encontrados ${candidates.length} candidatos pr\xF3ximos ao servi\xE7o ${serviceId}`);
  const totalAmount = serviceData.price_estimated;
  const providerAmount = calculateProviderAmount(totalAmount, commissionRate);
  LOG.despacho(`Valor total (Estimado): R$${totalAmount.toFixed(2)} | Valor l\xEDquido prestador (85%): R$${providerAmount.toFixed(2)}`);
  let insertedCount = 0;
  for (let i = 0; i < candidates.length; i++) {
    const candidate = candidates[i];
    const radiusKm = candidate.radius_km ?? 50;
    if (candidate.distance > radiusKm) {
      LOG.despacho(`Prestador ${candidate.user_id} ignorado: dist\xE2ncia ${candidate.distance.toFixed(2)}km > raio m\xE1ximo ${radiusKm}km`);
      continue;
    }
    try {
      await db.prepare(`
      ...
      insertedCount++;
      LOG.despacho(`\u2705 Prestador ${candidate.user_id} adicionado \xE0 fila (posi\xE7\xE3o ${i + 1}, dist\xE2ncia ${candidate.distance.toFixed(2)}km)`);
    } catch (error) {
  ...
  LOG.sucesso(`Fila populada com ${insertedCount} candidatos para o servi\xE7o ${serviceId}`);
  return insertedCount;
}
__name(populateServiceQueue, "populateServiceQueue");
async function updateNotificationStatus(db, serviceId, providerUserId, status, incrementCount = false) {
...
__name(updateNotificationStatus, "updateNotificationStatus");
var DispatchManager = class {
  ...
  async fetch(request) {
    ...
    if (path === "/wake-up") {
      LOG.despacho(`\u{1F514} Despertando Despachante Global para verificar servi\xE7os pendentes...`);
      const currentAlarm = await this.state.storage.getAlarm();
      if (currentAlarm === null) {
        LOG.despacho(`\u{1F680} Iniciando ciclo de despacho imediatamente (nenhum alarme ativo)`);
        await this.state.storage.setAlarm(Date.now());
        return new Response("Dispatcher Started");
      }
      LOG.despacho(`\u23F0 Alarme j\xE1 agendado para ${new Date(currentAlarm).toLocaleTimeString()}`);
  ...
  async alarm() {
    ...
    try {
      ...
      try {
        const config = await db.prepare("SELECT value FROM app_config WHERE key = 'dispatch_max_cycles'").first();
        if (config) maxCyclesRecycle = parseInt(config.value);
      } catch (e) {
      }
      const stalled = await db.prepare(`
                SELECT id FROM service_requests_new 
                WHERE status IN ('pending', 'paid', 'offered')
                  ...
                  AND EXISTS (
                      ...
                      WHERE service_id = service_requests_new.id 
                        AND status = 'SILENCE' 
                        AND ciclo_atual < ?
                  )
            `).bind(maxCyclesRecycle).all();
      if (stalled.results?.length > 0) {
        LOG.despacho(`\u267B\uFE0F Encontrados ${stalled.results.length} servi\xE7os com prestadores aguardando reciclagem.`);
        for (const svc of stalled.results) {
          LOG.despacho(`\u267B\uFE0F Reciclando fila para o servi\xE7o ${svc.id} (reativando SILENCE -> PENDING)`);
          await db.prepare(`
                        UPDATE notificacao_de_servicos 
      ...
      if (stuckServices.results?.length > 0) {
        LOG.warn(`Detectados ${stuckServices.results.length} servi\xE7os travados no alarme. For\xE7ando avan\xE7o na fila...`);
        for (const svc of stuckServices.results) {
          await db.prepare(`
      ...
      if (nextInterval > 0) {
        LOG.despacho(`\u23F3 Pr\xF3ximo ciclo agendado para daqui a ${nextInterval}s`);
        await this.state.storage.setAlarm(Date.now() + nextInterval * 1e3);
      } else {
        LOG.despacho(`\u{1F634} Sil\xEAncio total: Sem servi\xE7os pendentes ou zumbis. Despachante dormindo.`);
      }
    } catch (error) {
      LOG.erro(`Erro cr\xEDtico no ciclo de despacho:`, error);
      await this.state.storage.setAlarm(Date.now() + 6e4);
    }
  }
  async runGlobalDispatchCycle(db, env) {
    LOG.despacho(`\u{1F981}=== INICIANDO CICLO DE DESPACHO ===`);
    let maxCycles = 15;
    ...
    try {
      ...
      LOG.despacho(`Configura\xE7\xF5es: m\xE1ximo ${maxCycles} ciclos | intervalo ${cycleInterval}s`);
    } catch (e) {
      LOG.erro(`Erro ao carregar configura\xE7\xF5es de despacho:`, e);
    }
    const { results: pendingServices } = await db.prepare(`
      SELECT DISTINCT ns.service_id, sr.status, sr.notification_attempts, sr.created_at
      FROM notificacao_de_servicos ns
      JOIN service_requests_new sr ON ns.service_id = sr.id
    ...
    if (!pendingServices || pendingServices.length === 0) {
      const activeServicesQuery = await db.prepare(`
                SELECT id, status, profession, profession_id, price_estimated, latitude, longitude, notification_attempts 
                FROM service_requests_new 
                WHERE status IN ('pending', 'paid', 'offered') 
                  AND provider_id IS NULL
            `).all();
      ...
      if (activeServices.length > 0) {
        ...
        for (const service of activeServices) {
          ...
          const schedulingThreshold = 20;
          if (currentAttempt >= schedulingThreshold) {
            LOG.warn(`\u{1F6D1} Servi\xE7o ${service.id} atingiu o limite de ${schedulingThreshold} ciclos. Transicionando para AGENDAMENTO.`);
            await db.prepare(`
                            UPDATE service_requests_new 
            ...
            await db.prepare(`
                        ...
                        `).bind(service.id).run();
            LOG.sucesso(`\u{1F4C5} Servi\xE7o ${service.id} agora est\xE1 dispon\xEDvel para Agendamento Futuro.`);
            continue;
          }
          if (countRes?.c === 0) {
            LOG.warn(`\u{1F9DF} Servi\xE7o Zumbi detectado: ${service.id}. Ciclo ${currentAttempt} de ${maxCycles}.`);
            try {
              const inserted = await populateServiceQueue(db, service.id, service);
              if (inserted > 0) {
                servicesRescued++;
                LOG.sucesso(`\u{1F691} Servi\xE7o ${service.id} resgatado com ${inserted} prestadores.`);
              } else {
                LOG.warn(`\u26A0\uFE0F Servi\xE7o ${service.id} continua sem prestadores dispon\xEDveis. Incrementando tentativa.`);
                await db.prepare("UPDATE service_requests_new SET notification_attempts = notification_attempts + 1, last_notification_at = datetime('now') WHERE id = ?").bind(service.id).run();
              }
            } catch (e) {
              LOG.erro(`Erro ao tentar resgatar servi\xE7o ${service.id}:`, e);
            }
          } else {
            LOG.despacho(`\u23F3 Servi\xE7o ${service.id} aguardando reciclagem. Ciclo ${currentAttempt} de ${maxCycles}.`);
            await db.prepare("UPDATE service_requests_new SET notification_attempts = notification_attempts + 1, last_notification_at = datetime('now') WHERE id = ?").bind(service.id).run();
          }
        }
        if (servicesRescued > 0) {
          LOG.sucesso(`\u{1F691} Resgatados ${servicesRescued} servi\xE7os zumbis. Reiniciando ciclo imediatamente.`);
          return 1;
        }
        LOG.despacho(`\u2705 Nenhum servi\xE7o aguardando despacho (existentes: ${activeServices.length}, mas sem prestadores dispon\xEDveis). Entrando em backoff.`);
        return 30;
      } else {
        LOG.despacho(`\u{1F634} Sem servi\xE7os pendentes ou ativos. Parando alarme.`);
        return 0;
      }
    }
    LOG.despacho(`\u2699\uFE0F Processando ${pendingServices.length} servi\xE7o(s) na fila`);
    for (const serviceRow of pendingServices) {
      const serviceId = serviceRow.service_id;
      const currentAttempt = (serviceRow.notification_attempts || 0) + 1;
      const schedulingThreshold = 20;
      if (currentAttempt >= schedulingThreshold) {
        LOG.warn(`\u{1F6D1} Servi\xE7o ${serviceId} atingiu o limite de ${schedulingThreshold} ciclos. Transicionando para AGENDAMENTO.`);
        await db.prepare(`
                    UPDATE service_requests_new 
                    SET status = 'open_for_schedule' 
                    WHERE id = ?
        ...
        await db.prepare(`
                    DELETE FROM notificacao_de_servicos WHERE service_id = ?
                `).bind(serviceId).run();
        LOG.sucesso(`\u{1F4C5} Servi\xE7o ${serviceId} agora est\xE1 dispon\xEDvel para Agendamento Futuro.`);
        continue;
      }
      LOG.despacho(`\u{1F527}=== PROCESSANDO SERVI\xC7O ${serviceId} (Tentativa Global ${currentAttempt}) ===`);
      const pendingCandidates = await db.prepare(`
      ...
      if (!pendingCandidates.results || pendingCandidates.results.length === 0) {
        LOG.warn(`Nenhum candidato pendente encontrado para o servi\xE7o ${serviceId}. Pulando...`);
        continue;
      }
      for (const nextCandidate of pendingCandidates.results) {
        LOG.despacho(`\u{1F4CD} Verificando candidato: Prestador #${nextCandidate.provider_user_id} (posi\xE7\xE3o ${nextCandidate.queue_order})`);
        if (nextCandidate.last_notified_at) {
          const lastNotified = new Date(nextCandidate.last_notified_at);
          const now = /* @__PURE__ */ new Date();
          const secondsSinceLast = (now.getTime() - lastNotified.getTime()) / 1e3;
          if (secondsSinceLast < cycleInterval) {
            LOG.despacho(`\u23F8\uFE0F Prestador #${nextCandidate.provider_user_id} notificado h\xE1 ${secondsSinceLast.toFixed(0)}s. Aguardando pr\xF3ximo ciclo...`);
            break;
          } else {
            LOG.despacho(`\u23F1\uFE0F Timeout atingido para Prestador #${nextCandidate.provider_user_id}. Marcando como SILENCE.`);
        ...
        try {
          ...
          LOG.notificacao(`\u{1F4E4} Enviando notifica\xE7\xE3o para Prestador #${nextCandidate.provider_user_id}`);
          LOG.notificacao(`   Servi\xE7o: ${nextCandidate.service_name}`);
          LOG.notificacao(`   Dist\xE2ncia: ${distanceKm}km | Valor l\xEDquido: ${priceFormatted}`);
          ...
          const fcmResult = await sendFCMNotificationV1(
            ...
            {
              title: "Novo Servi\xE7o Dispon\xEDvel! \u{1F680}",
              body: `Servi\xE7o de ${nextCandidate.service_name} (Receba: ${priceFormatted}) a ${distanceKm}km.`,
              data: {
                type: "new_service",
                id: serviceId,
          ...
          if (fcmResult) {
            LOG.sucesso(`\u2705 Notifica\xE7\xE3o entregue ao Prestador #${nextCandidate.provider_user_id}`);
            await updateNotificationStatus(
              db,
              serviceId,
              nextCandidate.provider_user_id,
              "PENDING",
              true
            );
            await db.prepare(`
            ...
            WHERE id = ?
          `).bind(serviceId).run();
            LOG.despacho(`\u{1F4CA} Servi\xE7o ${serviceId} atualizado: tentativa #${serviceRow.notification_attempts + 1}`);
            break;
          } else {
            LOG.warn(`\u26A0\uFE0F Notifica\xE7\xE3o N\xC3O entregue ao Prestador #${nextCandidate.provider_user_id}. Token pode estar inv\xE1lido.`);
            await updateNotificationStatus(db, serviceId, nextCandidate.provider_user_id, "REJECTED");
          }
        } catch (error) {
          LOG.erro(`Falha ao enviar notifica\xE7\xE3o para Prestador #${nextCandidate.provider_user_id}:`, error.message);
          if (error.message?.includes("invalid registration token") || error.message?.includes("not registered")) {
            await updateNotificationStatus(db, serviceId, nextCandidate.provider_user_id, "REJECTED");
            LOG.warn(`\u{1F5D1}\uFE0F Token inv\xE1lido. Prestador #${nextCandidate.provider_user_id} removido da fila.`);
          }
        }
      }
    }
  ...
  try {
    ...
    if (action === "ACCEPTED" || action === "REJECTED") {
      await db.prepare(`
            ...
            `).bind(providerId).run();
      console.log(`[Log\xEDstica] \u{1F513} Trava liberada para o Prestador ${providerId} devido a ${action}`);
    }
    ...
    if (userRole === "provider") {
      try {
        ...
        services = services.map((s) => ({
          ...
          original_price: s.price_upfront || s.price_estimated,
          // Keep original if needed for history
          commission_rate: rate
        }));
      } catch (e) {
        console.error("[Servi\xE7os] \u274C Erro ao calcular comiss\xE3o:", e);
      }
    }
    console.log(`[Servi\xE7os] \u{1F4C4} Usu\xE1rio ${userId} (${userRole}) possui ${services.length} servi\xE7os`);
    return c.json({
      success: true,
      services
    });
  } catch (error) {
    console.error("[Servi\xE7os] \u274C Erro ao buscar hist\xF3rico:", error.message);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.get("/api/services/available", async (c) => {
  try {
    ...
    console.log(`[Servi\xE7os] \u{1F50D} Buscando servi\xE7os dispon\xEDveis para as profiss\xF5es do Prestador ${userId}`);
    ...
    let services = result.results || [];
    console.log(`[Servi\xE7os] \u{1F4CD} Encontrados ${services.length} servi\xE7os antes do filtro de proximidade`);
    ...
    if (location && location.latitude && location.longitude) {
      services = services.filter((s) => {
        ...
        console.log(`[Servi\xE7os] \u{1F4CF} Servi\xE7o ${s.id} est\xE1 a ${distance.toFixed(2)}km`);
    ...
    console.log(`[Servi\xE7os] \u2705 Retornando ${services.length} servi\xE7os ap\xF3s filtro de proximidade`);
    return c.json({ success: true, services });
  } catch (error) {
    console.error("[Servi\xE7os] \u274C Erro ao buscar servi\xE7os dispon\xEDveis:", error.message);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.get("/api/services/available-for-schedule", async (c) => {
  try {
    ...
    console.log(`[Agendamento] \u2705 Encontrados ${services.length} servi\xE7os dispon\xEDveis para agendamento`);
    return c.json({ success: true, services });
  } catch (error) {
    console.error("[Agendamento] \u274C Erro ao buscar servi\xE7os para agendamento:", error.message);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.post("/api/services/:id/propose-schedule", async (c) => {
  ...
  const authHeader = c.req.header("Authorization");
  const db = c.env.DB;
  if (!scheduled_at) return c.json({ success: false, message: "Missing scheduled_at" }, 400);
  try {
    let providerId = null;
    if (authHeader && authHeader.startsWith("Bearer ")) {
      const token = authHeader.substring(7);
    ...
    const service = await db.prepare("SELECT status FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    if (service.status !== "open_for_schedule" && service.status !== "schedule_proposed") {
      return c.json({ success: false, message: "Service not open for scheduling" }, 400);
    }
    await db.prepare(`
            UPDATE service_requests_new
            SET status = 'schedule_proposed',
                provider_id = ?,
                scheduled_at = ?
            WHERE id = ?
        `).bind(providerId, scheduled_at, serviceId).run();
    c.executionCtx.waitUntil((async () => {
      const s = await db.prepare("SELECT client_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
      if (s) {
        await syncStatusToFirebase(c.env, serviceId, "schedule_proposed", [s.client_id, providerId], { scheduled_at });
      }
    })());
    LOG.sucesso(`\u{1F4C5} Proposta de agendamento enviada por Prestador ${providerId} para Servi\xE7o ${serviceId} em ${scheduled_at}`);
    return c.json({ success: true });
  } catch (e) {
    LOG.erro(`Erro ao propor agendamento:`, e);
    return c.json({ success: false, error: e.message }, 500);
  }
});
app.post("/api/services/:id/confirm-schedule", async (c) => {
  ...
  const db = c.env.DB;
  try {
    const service = await db.prepare("SELECT status, provider_id, client_id, scheduled_at FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    if (service.status !== "schedule_proposed") {
      return c.json({ success: false, message: "No schedule proposal to confirm" }, 400);
    }
    await db.prepare(`
            UPDATE service_requests_new
        ...
        `).bind(serviceId).run();
    c.executionCtx.waitUntil((async () => {
      await syncStatusToFirebase(c.env, serviceId, "scheduled", [service.client_id, service.provider_id], { scheduled_at: service.scheduled_at });
    })());
    LOG.sucesso(`\u{1F4C5} Agendamento CONFIRMADO para Servi\xE7o ${serviceId}`);
    return c.json({ success: true });
  } catch (e) {
...
app.post("/api/services", async (c) => {
  ...
  try {
    ...
    if (bypassEmail) {
      console.log(`[Solicita\xE7\xE3o] \u{1F464} Usando email de bypass: ${bypassEmail}`);
      ...
      if (decoded && decoded.email) {
        ...
        if (user) {
          clientId = user.id;
          console.log(`[Solicita\xE7\xE3o] \u{1F464} Usando ID de usu\xE1rio autenticado: ${clientId}`);
        } else {
          console.log(`[Solicita\xE7\xE3o] \u2728 Usu\xE1rio n\xE3o encontrado, criando: ${decoded.email}`);
          ...
          if (newUser) {
            clientId = newUser.id;
            console.log(`[Solicita\xE7\xE3o] \u2728 Novo usu\xE1rio criado com ID: ${clientId}`);
          }
        }
      }
    }
  } catch (authError) {
    console.error("[Solicita\xE7\xE3o] \u274C Erro de autentica\xE7\xE3o:", authError.message);
  }
  ...
  if (task_id) {
    try {
      ...
      if (t) {
        ...
        console.warn(`[Solicita\xE7\xE3o] \u26A0\uFE0F ID da Tarefa ${task_id} n\xE3o encontrado.`);
      }
    } catch (e) {
      console.error("[Solicita\xE7\xE3o] \u274C Erro ao validar task_id:", e);
    }
  }
  try {
    const db = c.env.DB;
    ...
    console.log(`[Solicita\xE7\xE3o] \u2705 Servi\xE7o ${serviceId} criado aguardando pagamento. Valor: ${price_estimated}`);
    return c.json({
      success: true,
      service: {
        id: serviceId,
...
app.get("/api/service/:id/tracking", async (c) => {
  ...
  try {
    const service = await db.prepare(
      "SELECT status, profession, arrived_at FROM service_requests_new WHERE id = ?"
    ...
    const { results: history } = await db.prepare(`
        ...
        `).bind(serviceId).all();
    let headline = "Processando sua solicita\xE7\xE3o...";
    const timeline = [];
    const notifiedProviders = /* @__PURE__ */ new Set();
      ...
      switch (event.action) {
        case "NOTIFIED":
          ...
          if (notifiedProviders.size > 0) {
            timeline.push({ time, message: `${lastProviderName} n\xE3o respondeu. Buscando pr\xF3ximo...` });
          } else {
          ...
          break;
        case "REJECTED":
          timeline.push({ time, message: `${providerName} est\xE1 ocupado. Buscando pr\xF3ximo...` });
          break;
        case "ACCEPTED":
          timeline.push({ time, message: `\u2705 ${providerName} aceitou! Ele j\xE1 est\xE1 visualizando seu endere\xE7o.` });
          headline = `${providerName} est\xE1 a caminho!`;
          break;
      }
    }
    ...
    if (isSearching) {
      if (lastNotificationTime) {
        const now = /* @__PURE__ */ new Date();
        const diffSeconds = (now.getTime() - lastNotificationTime.getTime()) / 1e3;
      ...
      headline = "Servi\xE7o finalizado";
    }
    if (timeline.length === 0 && isSearching) {
      timeline.push({ time: "Agora", message: "Procurando prestadores dispon\xEDveis..." });
    }
    if (service.arrived_at) {
      timeline.push({ time: "Agora", message: "\u{1F4CD} O prestador chegou ao seu endere\xE7o." });
    }
    return c.json({
      success: true,
...
app.post("/api/services/ai/classify", async (c) => {
  ...
  try {
    ...
    console.log(`[IA] \u{1F9E0} Resposta mapeada: ${mappedResponse.profissao} (Encontrado: ${mappedResponse.encontrado})`);
    return c.json(mappedResponse);
  } catch (error) {
    console.error(`[IA] \u274C Erro de classifica\xE7\xE3o: ${error.message}`);
    return c.json({ success: false, error: error.message }, 500);
  }
});
...
app.get("/api/profile/specialties", async (c) => {
  try {
    ...
    console.log(`[Especialidades] \u{1F393} Prestador ${userId} possui ${specialties.length} especialidades`);
    return c.json({
      success: true,
      specialties
    });
...
app.delete("/api/profile/specialties", async (c) => {
  ...
  try {
    ...
    console.log(`[Especialidades] \u{1F5D1}\uFE0F Removida ${name} do prestador ${userId}`);
...
app.get("/api/profile/me", async (c) => {
  try {
    ...
    if (user.role === "provider") {
      providerData = await db.prepare("SELECT wallet_balance, commercial_name FROM providers WHERE user_id = ?").bind(userId).first();
    }
    return c.json({
      success: true,
      user: {
...
app.get("/api/services/:id", async (c) => {
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      const decoded = decodeJwt(token);
      if (decoded && decoded.email) {
        const u = await db.prepare("SELECT role FROM users WHERE email = ?").bind(decoded.email).first();
    ...
    if (userRole === "provider") {
      ...
      service = {
        ...
        price_upfront: service.price_upfront ? service.price_upfront * commissionFactor : service.price_estimated ? service.price_estimated * commissionFactor : null,
        platform_commission: platformCommissionRate
      };
    }
    let provider = null;
...
app.post("/api/services/:id/cancel", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const service = await db.prepare("SELECT status, arrived_at, provider_id, client_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    if (service.arrived_at) {
      LOG.warn(`\u{1F6AB} Tentativa de cancelamento bloqueada para servi\xE7o ${serviceId}: Prestador j\xE1 chegou.`);
      return c.json({
        success: false,
        message: "N\xE3o \xE9 poss\xEDvel cancelar ap\xF3s a chegada do prestador. Entre em contato com o suporte se houver problemas."
      }, 403);
    }
    ...
    c.executionCtx.waitUntil(
      db.prepare("UPDATE notificacao_de_servicos SET status = 'CANCELLED' WHERE service_id = ?").bind(serviceId).run()
    );
    return c.json({ success: true, message: "Service cancelled" });
  } catch (error) {
    LOG.erro(`Erro ao cancelar servi\xE7o ${serviceId}:`, error);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.post("/api/services/:id/accept", async (c) => {
  ...
  try {
    ...
    if (!providerId) {
      LOG.warn(`Tentativa de aceitar servi\xE7o ${serviceId} sem autentica\xE7\xE3o`);
      return c.json({ success: false, message: "N\xE3o autorizado" }, 401);
    }
    LOG.servico(`\u{1F91D} Prestador #${providerId} ACEITOU servi\xE7o ${serviceId}`);
    const providerInfo = await db.prepare("SELECT full_name FROM users WHERE id = ?").bind(providerId).first();
    const clientInfo = await db.prepare("SELECT client_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    const result = await updateServiceStatus(
      c.env,
      serviceId,
      "accepted" /* ACCEPTED */,
      [clientInfo?.client_id, providerId],
      // Notifica cliente e o próprio prestador (para refresh)
      {
        provider_id: providerId,
        provider_name: providerInfo?.full_name || "Prestador"
      },
      { provider_id: providerId },
      // Extra column to update
      "id = ? AND status IN ('pending', 'offered')"
      // Specific where clause
    );
    if (result.meta.changes === 0) {
      LOG.warn(`Servi\xE7o ${serviceId} j\xE1 foi aceito por outro prestador ou cancelado`);
      return c.json({ success: false, message: "Servi\xE7o n\xE3o dispon\xEDvel" }, 409);
    }
    c.executionCtx.waitUntil((async () => {
      await db.prepare(`
                UPDATE notificacao_de_servicos
      ...
      LOG.despacho(`\u23F9\uFE0F Fila de notifica\xE7\xF5es atualizada para servi\xE7o ${serviceId}`);
    })());
    return c.json({ success: true, message: "Servi\xE7o aceito com sucesso" });
  } catch (error) {
    LOG.erro(`Falha ao aceitar servi\xE7o ${serviceId}:`, error);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.post("/api/services/:id/skip", async (c) => {
  ...
  try {
    ...
    if (providerId) {
      ...
      console.log(`[Log\xEDstica] \u{1F6AB} Servi\xE7o ${serviceId} recusado pelo prestador ${providerId}`);
    }
    ...
    console.error("[Log\xEDstica] \u274C Erro ao recusar servi\xE7o:", error.message);
    return c.json({ success: false, error: error.message }, 500);
  }
});
app.post("/api/services/:id/reject", async (c) => {
  ...
  try {
    ...
    const obj = c.env.DISPATCH_MANAGER.get(id);
    await obj.fetch(new Request(`http://dispatch/skip`, { method: "POST" }));
    console.log(`[Log\xEDstica] \u{1F6AB} Servi\xE7o ${serviceId} recusado pelo prestador`);
...
app.post("/api/services/:id/propose-schedule", async (c) => {
  ...
  try {
    ...
    if (!providerId) return c.json({ success: false, message: "N\xE3o autorizado" }, 401);
    if (!scheduled_at) return c.json({ success: false, message: "Data/Hora necess\xE1ria" }, 400);
    await updateServiceStatus(
      c.env,
      serviceId,
      "schedule_proposed",
      ...
      {},
      { provider_id: providerId, scheduled_at },
      "id = ? AND status IN ('open_for_schedule', 'schedule_proposed', 'pending', 'paid', 'offered')"
    );
    c.executionCtx.waitUntil((async () => {
      const service = await db.prepare("SELECT client_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
      if (service) {
        await syncStatusToFirebase(c.env, serviceId, "schedule_proposed", [service.client_id, providerId], {
          scheduled_at,
...
app.post("/api/services/:id/confirm-schedule", async (c) => {
  ...
  const db = c.env.DB;
  try {
    const service = await db.prepare("SELECT provider_id, client_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Servi\xE7o n\xE3o encontrado" }, 404);
    await updateServiceStatus(
      c.env,
      serviceId,
      "scheduled",
...
app.post("/api/services/:id/arrive", async (c) => {
  ...
  try {
    ...
    const info = await db.prepare(`
            SELECT sr.client_id, u.fcm_token 
            FROM service_requests_new sr
            JOIN users u ON sr.client_id = u.id
            WHERE sr.id = ?
        `).bind(serviceId).first();
    if (!info) return c.json({ success: false, message: "Service/Client not found" }, 404);
    const arrivedAt = (/* @__PURE__ */ new Date()).toISOString();
    await updateServiceStatus(
      c.env,
      serviceId,
      "accepted",
      [info.client_id, providerId].filter((id) => id !== null),
    ...
    try {
      const id = c.env.DISPATCH_MANAGER.idFromName(serviceId);
      const obj = c.env.DISPATCH_MANAGER.get(id);
      await obj.fetch(new Request("http://dispatch/broadcast", {
        method: "POST",
        body: JSON.stringify({
          type: "service.updated",
          data: { id: serviceId, status: "accepted", arrived_at: arrivedAt }
        })
      }));
    } catch (e) {
      console.error("[Chegada] \u274C Erro em tempo real:", e);
    }
    if (info && info.fcm_token && c.env.FIREBASE_SERVICE_ACCOUNT) {
      await sendFCMNotificationV1(c.env.FIREBASE_SERVICE_ACCOUNT, info.fcm_token, {
        title: "Prestador Chegou! \u{1F4CD}",
        body: "Por favor, realize o pagamento do restante para iniciar o servi\xE7o.",
        data: {
    ...
    console.log(`[Chegada] \u{1F4CD} Prestador #${providerId || "?"} chegou para o servi\xE7o ${serviceId}`);
    return c.json({ success: true, message: "Arrival recorded" });
    console.log(`[Chegada] \u{1F4CD} Prestador #${providerId || "?"} chegou para o servi\xE7o ${serviceId}`);
    return c.json({ success: true, message: "Arrival recorded" });
  } catch (error) {
    return c.json({ success: false, error: error.message }, 500);
...
app.post("/api/services/:id/start", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const service = await db.prepare("SELECT client_id, provider_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    ...
    console.log(`[Execu\xE7\xE3o] \u{1F680} Servi\xE7o ${serviceId} iniciado`);
...
app.post("/api/services/:id/verify-code", async (c) => {
  ...
  try {
    ...
    const db = c.env.DB;
    const service = await db.prepare("SELECT completion_code FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
...
app.post("/api/services/:id/confirm-completion", async (c) => {
  ...
  try {
    ...
    const db = c.env.DB;
    const service = await db.prepare("SELECT completion_code, client_id, provider_id, status, provider_amount, price_estimated FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    ...
    await updateServiceStatus(
      ...
      { finished_at: finishedAt, proof_video }
    );
    if (amountToRelease && service.provider_id) {
      ...
      await db.prepare(`
        ...
        serviceId,
        amountToRelease,
        `Ganho pelo servi\xE7o #${serviceId.substring(0, 8)}`
      ).run();
      console.log(`[Financeiro] \u{1F4B8} Saldo de R$ ${amountToRelease} liberado para Prestador #${service.provider_id}`);
    }
    c.executionCtx.waitUntil((async () => {
      try {
        const clientUser = await db.prepare("SELECT fcm_token FROM users WHERE id = ?").bind(service.client_id).first();
        if (clientUser?.fcm_token && c.env.FIREBASE_SERVICE_ACCOUNT) {
          await sendFCMNotificationV1(c.env.FIREBASE_SERVICE_ACCOUNT, clientUser.fcm_token, {
            ...
            body: "O prestador finalizou o servi\xE7o com sucesso. Obrigado!",
...
app.post("/api/services/:id/complete", async (c) => {
  ...
  try {
    ...
    await db.prepare(`
            UPDATE service_requests_new 
            SET status = '${"completed" /* COMPLETED */}', finished_at = datetime('now')
            WHERE id = ?
        `).bind(serviceId).run();
    c.executionCtx.waitUntil((async () => {
      try {
        ...
        LOG.erro(`Falha no sync p\xF3s-conclus\xE3o: ${err.message}`);
      }
    })());
...
app.post("/api/services/:id/pay_remaining", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const payment = await db.prepare("SELECT id FROM payments WHERE mission_id = ? AND status = 'approved'").bind(serviceId).first();
    if (!payment) {
    ...
    await db.prepare(`
            UPDATE service_requests_new 
            SET status = '${"in_progress" /* IN_PROGRESS */}', 
                payment_remaining_status = 'paid',
                started_at = datetime('now')
            WHERE id = ?
        `).bind(serviceId).run();
    const service = await db.prepare("SELECT provider_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (service && service.provider_id && c.env.FIREBASE_SERVICE_ACCOUNT) {
      const provider = await db.prepare("SELECT fcm_token FROM users WHERE id = ?").bind(service.provider_id).first();
      if (provider && provider.fcm_token) {
        ...
        console.log(`[Pagamento] \u{1F514} Notifica\xE7\xE3o enviada ao prestador ${service.provider_id}`);
      }
    }
    c.executionCtx.waitUntil((async () => {
      try {
        const s = await db.prepare("SELECT client_id, provider_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
        if (s) {
          await syncStatusToFirebase(
            c.env,
            serviceId,
        ...
        LOG.erro(`Falha no sync p\xF3s-pagamento: ${err.message}`);
      }
    })());
    console.log(`[Pagamento] \u2705 Saldo restante pago para o servi\xE7o ${serviceId}`);
...
app.post("/api/notifications/register-token", async (c) => {
  ...
  try {
    ...
    return c.json({ success: false, message: "JSON inv\xE1lido" }, 400);
  }
  const { token, platform, latitude, longitude } = body;
  if (!token) return c.json({ success: false, message: "Token obrigat\xF3rio" }, 400);
  const db = c.env.DB;
  try {
    const authHeader = c.req.header("Authorization");
    const bypassEmail = c.req.header("X-Test-Bypass-Email");
    ...
    let userRole = null;
    if (bypassEmail) {
      const u = await db.prepare("SELECT id, role FROM users WHERE email = ?").bind(bypassEmail).first();
      if (u) {
        userId = u.id;
        userRole = u.role;
      }
      ...
      if (decoded?.email) {
        const u = await db.prepare("SELECT id, fcm_token, role FROM users WHERE email = ?").bind(decoded.email).first();
        if (u) {
          userId = u.id;
          userRole = u.role;
        }
    ...
    if (!userId) return c.json({ success: false, message: "N\xE3o autorizado" }, 401);
    await db.prepare("UPDATE users SET fcm_token = ? WHERE id = ?").bind(token, userId).run();
    LOG.notificacao(`\u{1F4F1} Token FCM registrado para usu\xE1rio #${userId} (${platform})`);
    ...
    if (userRole === "provider") {
      ...
      const professionIds = (profs.results || []).length > 0 ? JSON.stringify((profs.results || []).map((r) => r.id)) : null;
      ...
      await db.prepare(`
        INSERT INTO notification_registry (
          ...
          latitude, longitude, is_online, last_seen_at
      ...
      LOG.sucesso(`\u2705 Prestador #${userId} agora est\xE1 ONLINE e dispon\xEDvel para despacho`);
      LOG.despacho(`   Profiss\xF5es: ${professionList || "nenhuma"}`);
      if (finalLat && finalLon) {
        LOG.despacho(`   Localiza\xE7\xE3o: ${finalLat.toFixed(4)}, ${finalLon.toFixed(4)}`);
      }
    } else {
      LOG.notificacao(`\u2139\uFE0F Usu\xE1rio #${userId} \xE9 cliente - registro limitado a notifica\xE7\xF5es b\xE1sicas`);
    }
...
app.delete("/api/notifications/token", async (c) => {
  ...
  try {
    ...
    if (userId) {
      ...
      console.log(`[FCM] \u{1F6AA} Usu\xE1rio ${userId} REMOVIDO do registro (Logout)`);
    } else if (token) {
      await db.prepare("DELETE FROM notification_registry WHERE fcm_token = ?").bind(token).run();
      console.log(`[FCM] \u{1F5D1}\uFE0F Token ${token.substring(0, 10)}... REMOVIDO do registro`);
    }
...
app.post("/api/payment/process", async (c) => {
  ...
  try {
    ...
    const service = await db.prepare("SELECT profession, price_estimated, price_upfront FROM service_requests_new WHERE id = ?").bind(service_id).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
...
app.post("/api/payment/webhook", async (c) => {
  ...
  LOG.pagamento(`\u{1F4E5} Webhook recebido: type=${eventType} | payment_id=${paymentId}`);
  if ((eventType === "payment" || eventType === "payment.updated") && paymentId) {
    try {
      ...
      if (!response.ok) {
        LOG.erro(`Falha na verifica\xE7\xE3o do pagamento ${paymentId} com MP API`);
        return c.text("Verification failed", 200);
      }
      const paymentInfo = await response.json();
      const pStatus = paymentInfo.status;
      const externalRef = paymentInfo.external_reference;
      LOG.pagamento(`\u{1F4B0} Pagamento ${paymentId}: status=${pStatus} | ref=${externalRef}`);
      if (pStatus === "approved" && externalRef?.startsWith("SERVICE-")) {
        const serviceId = externalRef.replace("SERVICE-", "");
        LOG.pagamento(`\u2705 Pagamento APROVADO para servi\xE7o ${serviceId}`);
        await db.prepare("UPDATE payments SET status = ? WHERE mp_payment_id = ?").bind("approved", String(paymentId)).run();
        LOG.pagamento(`\u{1F4DD} Registro de pagamento atualizado para ${serviceId}`);
        const service = await db.prepare("SELECT status, provider_id FROM service_requests_new WHERE id = ?").bind(serviceId).first();
        if (!service) {
          LOG.erro(`Servi\xE7o ${serviceId} n\xE3o encontrado no banco`);
          return c.text("OK", 200);
        }
        LOG.pagamento(`Estado atual do servi\xE7o ${serviceId}: ${service.status}`);
        let updateSql = "";
        let params = [];
        let transitionMsg = "";
        if (service.status === "waiting_payment") {
          ...
          if (newStatus === "pending") {
            c.executionCtx.waitUntil((async () => {
              const updatedService = await db.prepare("SELECT * FROM service_requests_new WHERE id = ?").bind(serviceId).first();
              if (updatedService) {
                const queueCount = await populateServiceQueue(db, serviceId, updatedService);
                LOG.despacho(`\u{1F680} Fila populada com ${queueCount} prestadores para servi\xE7o ${serviceId}`);
                const id = c.env.DISPATCH_MANAGER.idFromName("GLOBAL_DISPATCHER");
                const obj = c.env.DISPATCH_MANAGER.get(id);
                await obj.fetch(new Request(`http://dispatch/wake-up`, { method: "POST" }));
                LOG.despacho(`\u{1F514} Despachante global acionado para servi\xE7o ${serviceId}`);
              }
            })());
          }
        } else if (service.status === "waiting_payment_remaining" || service.status === "accepted" && service.arrived_at !== null) {
          updateSql = "UPDATE service_requests_new SET status = ?, payment_remaining_status = ? WHERE id = ?";
          params = ["in_progress" /* IN_PROGRESS */, "paid", serviceId];
          transitionMsg = `\u2192 ${"in_progress" /* IN_PROGRESS */} (pagamento ${service.status === "accepted" /* ACCEPTED */ ? "restante " : ""}aprovado)`;
        }
        if (updateSql) {
          ...
          await updateServiceStatus(
            ...
            serviceId,
            newStatus,
            [service.client_id, service.provider_id].filter((id) => id !== null),
            { provider_id: service.provider_id },
            extra
          );
          LOG.sucesso(`\u{1F504} Servi\xE7o ${serviceId} ${transitionMsg}`);
          if (newStatus === "pending") {
            c.executionCtx.waitUntil((async () => {
              const updatedService = await db.prepare("SELECT * FROM service_requests_new WHERE id = ?").bind(serviceId).first();
              if (updatedService) {
          ...
          LOG.warn(`Nenhuma transi\xE7\xE3o necess\xE1ria para servi\xE7o ${serviceId} no estado ${service.status}`);
        }
      }
    } catch (error) {
      LOG.erro(`Erro cr\xEDtico no processamento do webhook:`, error);
    }
  }
  return c.text("OK", 200);
});
app.get("/api/payment/check/:serviceId", async (c) => {
  ...
  const mpToken = c.env.MP_ACCESS_TOKEN;
  try {
    LOG.pagamento(`\u{1F50D} Verifica\xE7\xE3o manual para o servi\xE7o: ${service_id}`);
    ...
    if (payment.mp_payment_id) {
      ...
      if (response.ok) {
        ...
        LOG.pagamento(`\u{1F4B0} Verifica\xE7\xE3o direta MP: ${mpInfo.status}`);
        if (mpInfo.status === "approved") {
          await db.prepare("UPDATE payments SET status = ? WHERE id = ?").bind("approved", payment.id).run();
          const service = await db.prepare("SELECT * FROM service_requests_new WHERE id = ?").bind(service_id).first();
          if (service && (service.status === "waiting_payment" || service.status === "waiting_payment_remaining")) {
            ...
            if (service.status === "waiting_payment_remaining") {
              await db.prepare("UPDATE service_requests_new SET status = ?, payment_remaining_status = ? WHERE id = ?").bind("in_progress", "paid", service_id).run();
              LOG.sucesso(`Servi\xE7o ${service_id} atualizado para EM ANDAMENTO (Pagamento final)`);
            } else {
              await db.prepare("UPDATE service_requests_new SET status = ? WHERE id = ?").bind(newStatus, service_id).run();
              LOG.sucesso(`Servi\xE7o ${service_id} aprovado para ${newStatus}`);
            }
            if (newStatus === "pending") {
              c.executionCtx.waitUntil((async () => {
                await populateServiceQueue(db, service_id, service);
                await triggerServiceNotifications(service_id, db, c.env, c.executionCtx);
                LOG.despacho(`\u{1F680} Fila populada e despacho iniciado para ${service_id}`);
              })());
    ...
    LOG.erro(`Erro na verifica\xE7\xE3o manual de pagamento ${service_id}:`, e);
    return c.json({ success: false, error: e.message }, 500);
  }
});
...
app.get("/api/debug/provider/:id", async (c) => {
  ...
  try {
    const user = await db.prepare("SELECT * FROM users WHERE id = ?").bind(id).first();
    const location = await db.prepare("SELECT * FROM provider_locations WHERE provider_id = ?").bind(id).first();
    const professions = await db.prepare(`
            SELECT p.name, pp.* 
            FROM provider_professions pp
            JOIN professions p ON pp.profession_id = p.id
            WHERE pp.provider_user_id = ?
...
app.post("/api/test/find-providers", async (c) => {
  try {
    ...
    const providers = await findProvidersByDistance(db, profession, latitude, longitude, 50);
    return c.json({
      success: true,
...
app.post("/api/payment/debug-confirm", async (c) => {
  try {
    ...
    LOG.pagamento(`\u2696\uFE0F [DEBUG] Aprovando servi\xE7o manualmente: ${serviceId}`);
    const service = await db.prepare("SELECT * FROM service_requests_new WHERE id = ?").bind(serviceId).first();
    if (!service) return c.json({ success: false, message: "Service not found" }, 404);
    let newStatus = service.status === "waiting_payment_remaining" ? "in_progress" /* IN_PROGRESS */ : service.provider_id ? "accepted" /* ACCEPTED */ : "pending" /* PENDING */;
    ...
    if (newStatus === "pending") {
      c.executionCtx.waitUntil((async () => {
        ...
        LOG.despacho(`\u{1F680} [DEBUG] Fila populada e motor acordado para o servi\xE7o ${serviceId}`);
      })());
    } else if (newStatus === "accepted") {
      await triggerServiceNotifications(serviceId, db, c.env, c.executionCtx);
    }
    ...
    LOG.erro(`Erro na aprova\xE7\xE3o manual debug:`, error);
    return c.json({ success: false, error: error.message }, 500);
  }
});
...
app.get("/api/strings/:language?", async (c) => {
  try {
    ...
    console.error("[Tradu\xE7\xE3o] \u274C Erro ao buscar strings:", error);
    return c.json({
      success: false,
      error: error.message
    }, 500);
...
app.get("/api/config", async (c) => {
  try {
    ...
    console.error("[Config] \u274C Erro ao buscar configura\xE7\xF5es:", error);
    return c.json({
      success: false,
      error: error.message
    }, 500);
...
var worker_default = {
  ...
  async scheduled(event, env, ctx) {
    LOG.sistema(`\u23F2\uFE0F Watchdog acionado via cron: ${event.cron || "manual"}`);
    const id = env.DISPATCH_MANAGER.idFromName("GLOBAL_DISPATCHER");
    const obj = env.DISPATCH_MANAGER.get(id);
    ctx.waitUntil(
