...
router.get("/me", authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (fullUser.role === "provider") {
      ...
      if (providerDetails?.wallet_balance) {
        balance = Number(providerDetails.wallet_balance);
      }
      professions = await userRepo.getProviderProfessions(fullUser.id!);
    }

    const { is_medical, is_fixed_location } = await userRepo.getProviderFlags(fullUser.id!);
