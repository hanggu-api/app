...
const _sanitize = (data: any) => {
  if (!data || typeof data !== 'object') return data;
  const clean: any = Array.isArray(data) ? [] : {};
  Object.keys(data).forEach(key => {
    if (data[key] !== undefined) {
      clean[key] = (typeof data[key] === 'object' && data[key] !== null && !(data[key] instanceof Date))
        ? _sanitize(data[key])
        : data[key];
    }
  });
...
export const FirebaseService = {
  ...
  updateServiceStatus: async (serviceId: string | number, data: any) => {
    ...
    try {
      ...
      await db.collection('services').doc(String(serviceId)).set(cleanData, { merge: true });
    } catch (e: any) {
  ...
  sendChatMessage: async (serviceId: string | number, message: any) => {
    ...
    try {
      const db = getFirestore();
      if (!db) return;
      const cleanMessage = _sanitize(message);
  ...
  verifyIdToken: async (token: string) => {
    try {
      // Simulation/Testing Bypass (DISABLED IN PRODUCTION)
      const isProduction = process.env.NODE_ENV === "production";
      const isTestSecret = token.startsWith("MOCK_TOKEN_SECRET_123_");

      if (!isProduction && (token.startsWith("MOCK_TOKEN_") || isTestSecret)) {
        const email = token.replace("MOCK_TOKEN_", "").replace("MOCK_TOKEN_SECRET_123_", "");
        return {
          uid: `mock_${email}`,
          email: email,
          name: "Mock User",
