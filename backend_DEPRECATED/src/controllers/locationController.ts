...
export const searchAddress = async (req: Request, res: Response) => {
  try {
    ...
    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(String(q))}&format=json&addressdetails=1&limit=5&countrycodes=br`;
    ...
    if (response.data && Array.isArray(response.data)) {
      const results = response.data.map((item: any) => {
        ...
        // Custom Formatting
        const parts = [
          addr.road || addr.pedestrian || addr.suburb, // Street
          addr.house_number, // Number
          addr.suburb || addr.neighbourhood, // Neighborhood
          addr.city || addr.town || addr.municipality, // City
          addr.state_code || addr.state, // State
          addr.postcode // Zip
        ...
        return {
          display_name: formatted || item.display_name, // Fallback to original if empty
          lat: item.lat,
          lon: item.lon,
