...
router.post('/:id/start', authMiddleware, async (req: Request, res: Response) => {
    try {
        const user = (req as AuthRequest).user;
        if (user?.role !== 'provider') {
            res.status(403).json({ success: false, message: 'Only providers can start services' });
            return;
        }

        const ok = await serviceRepo.updateStatus(req.params.id, 'in_progress');
        if (ok) {
            io.to(`service:${req.params.id}`).emit('service.in_progress', { id: req.params.id, provider_id: user.id });
            io.emit('service.status', { id: req.params.id, status: 'in_progress' });
...
router.post('/:id/complete', authMiddleware, async (req: Request, res: Response) => {
    try {
        const user = (req as AuthRequest).user;
        if (user?.role !== 'provider') {
            res.status(403).json({ success: false, message: 'Only providers can complete services' });
            return;
        }

        const ok = await serviceRepo.updateStatus(req.params.id, 'completed');
        if (ok) {
            io.to(`service:${req.params.id}`).emit('service.completed', { id: req.params.id, provider_id: user.id });
            io.emit('service.status', { id: req.params.id, status: 'completed' });
