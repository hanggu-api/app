...
async function runSimulation() {
    try {
        ...
        const found = available.data.services.find((s: any) => s.id === serviceId);
        if (found) {
        ...
        await axios.post(
            `${API_URL}/services/${serviceId}/accept`,
            {},
            { headers: { Authorization: `Bearer ${providerToken}` } }
        );
        ...
        await axios.post(`${API_URL}/chat/${serviceId}`, { content: 'Olá, pode vir hoje?' }, { headers: { Authorization: `Bearer ${clientToken}` } });
        // Provider replies
        await axios.post(`${API_URL}/chat/${serviceId}`, { content: 'Sim, chego em 30min.' }, { headers: { Authorization: `Bearer ${providerToken}` } });

        // Verify messages
        const messages = await axios.get(`${API_URL}/chat/${serviceId}`, { headers: { Authorization: `Bearer ${clientToken}` } });
    ...
    try {
        ...
        for (const c of clients) {
            for (let k = 0; k < servicesPerClient; k++) {
                ...
                const desc = `Serviço ${uuidv4().slice(0, 6)} descrição realista ${k}`;
        ...
        for (const s of services) {
            ...
            const verify = await axios.get(`${API_URL}/chat/${s.id}`, { headers: { Authorization: `Bearer ${client.token}` } });
